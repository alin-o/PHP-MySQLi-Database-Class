...
$activeUsers = $db->where("active", 1)->get(); // returns array of arrays (dehydrated)
// to get an instance of a user (hydrated)
$db->setModel("App\\Models\\User", "users", "id, username");
$user = $db->where("id", 1)->first();// returns User instance
// to get all the users (hydrated)
$db->setModel("App\\Models\\User", "users", "id, username");
$user = $db->where('active', 1)->all(); // returns array of User instances
```
...
Make sure you use \*_unlock()_ afterwards or your tables will remain locked!
